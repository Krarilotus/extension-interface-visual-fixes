...
function M.enable()
  ...
  local sites = {
    {A.TowerDoorCall1, layout.patterns.TowerDoorCall1},
    {A.TowerDoorCall2, layout.patterns.TowerDoorCall2},
    {A.TowerDoorCall3, layout.patterns.TowerDoorCall3},
    {A.TowerDoorCall4, layout.patterns.TowerDoorCall4},
    {A.TowerDoorCall5, layout.patterns.TowerDoorCall5},
  }
  ...
  local updateSites = {
    {A.RenderMapEntry, layout.patterns.RenderMapEntry},
    {A.FoundationDrawCall, layout.patterns.FoundationDrawCall},
    {A.BuildingRefresh, "89 9C 0F 94 02 00 00"},
    {A.BuildingRefreshTile, layout.patterns.BuildingRefreshTile, 6},
    {A.MapReset, layout.patterns.MapReset},
    {A.TowerFoundationSelect, layout.patterns.TowerFoundationSelect, 10},
  }
  ...
  local initializeDepth = layout.allocateAssembly(string.format([[
    mov eax, [%d]
    cmp [edi], eax
    je ready
    mov [edi], eax
  ...
  local deferredDraw = layout.allocateAssembly([[
    pushfd
    pushad
    ...
    add eax, 80
    shl eax, 4
    movzx eax, word [eax+ImageHeaders]
    add eax, ecx
    cmp [edi+4], eax
    jge immediate
    mov [edi+16], ecx
  ...
  immediate:
    popad
    popfd
    jmp DrawBuildingOverlay
  ]], initializeDepth))
  local foundationDraw = layout.allocateAssembly([[
    call DrawCliff
    pushfd
    pushad
    movzx esi, word [ebp*2+TileBuilding]
    test esi, esi
    jz done
    cmp esi, 2000
    jae done
    imul esi, esi, 812
    movzx eax, word [esi+BuildingKind]
    sub eax, 75
    cmp eax, 3
    ja done
  ]] .. depthAddress .. string.format([[
  ...
  paint_ready:
    ...
    mov ax, [edi+36]
    mov [ebp], ax
    mov dword [OverlayVerticalOffset], 0
    mov dword [OverlayGm], 54
    test dword [edi+12], 1
    jz paint_right
    mov eax, [edi+24]
    cmp [edi+4], eax
    ...
    push 81
    push 54
    mov ecx, TextureRenderer
    call DrawBuildingOverlay
  paint_right:
    test dword [edi+12], 2
  ...
  restore_draw_state:
    ...
    mov [ebp], ax
    pop dword [OverlayGm]
    pop dword [OverlayVerticalOffset]
  done:
    popad
    popfd
    ret
  ...
  local initialize = layout.allocateAssembly(address .. string.format([[
    mov eax, [esi+BuildingId]
    mov [edi], eax
    mov eax, [esi+BuildingTile]
    mov [edi+4], eax
    xor eax, eax
    mov [edi+8], eax
    mov [edi+12], eax
  ...
  -- -> EAX=rank, EDX=side.
  local rank = layout.allocateAssembly([[
    push ebx
    push ecx
    push esi
    push edi
    ...
    cmp eax, 80400
    jae absent
    mov edx, [eax*4+TileLogic]
    and edx, 0xB02
    cmp edx, 0x100
    jne absent
    movzx edi, byte [eax+TileHeight]
    mov ebp, [esi+BuildingTile]
    cmp ebp, 80400
    jae absent
    movzx ebp, byte [ebp+TileTerrain]
    cmp edi, ebp
    jae eligible
    mov eax, ebx
    xor edx, edx
    div ecx
    movzx ebx, word [esi+BuildingX]
    movzx ebp, word [esi+BuildingY]
    test eax, eax
    jz north_backing
    cmp eax, 1
    je east_backing
  ...
  backing_tile:
    ...
    cmp eax, 80400
    jae absent
    movzx eax, word [eax*2+TileBuilding]
    imul eax, eax, 812
    cmp eax, esi
    jne absent
    mov ebx, [esp+16]
  ...
  local beginUpdate = layout.allocateAssembly(string.format([[
    pushfd
    pushad
    mov esi, edi
    call %d
    ...
    jmp BuildingRefreshResume
  ]], initialize))
  local visitTile = layout.allocateAssembly(string.format([[
    pushfd
    pushad
    mov ecx, [edi+BuildingState]
    mov esi, edi
    call %d
    mov esi, edi
  ]], rank) .. address .. [[
  ...
  visited:
    popad
    popfd
    mov eax, [eax*4+TileLogic]
    jmp BuildingRefreshTileResume
  ]])
  ...
  local resetMap = layout.allocateAssembly(string.format([[
    pushfd
    inc dword [%d]
    popfd
    push ecx
  ...
  side_ready:
    ...
    cmp eax, 80400
    jae finish
    movzx eax, byte [eax+TileTerrain]
    add eax, 90
    mov [esp], eax
  ]] .. address .. [[
    mov eax, [%d]
  ...
  local foundation = layout.allocateAssembly([[
    pushfd
    pushad
    mov eax, [ebp-0x30]
    cmp eax, 2000
    jae original
    imul eax, eax, 812
    movzx eax, word [eax+BuildingKind]
    sub eax, 75
    cmp eax, 3
    ja original
    mov ecx, [ebp-0xE4]
  ...
  write_masonry:
    ...
    popad
    popfd
    jmp TowerFoundationResume
  original:
    popad
    popfd
    mov edx, [ebp-0xE4]
    jmp TowerFoundationOriginal
  ]])
  core.writeCode(A.FoundationDrawCall, {core.callTo(foundationDraw)})
  for _, site in ipairs(sites) do
    core.writeCode(site[1], {core.callTo(wrapper)})
  end
  core.writeCode(A.BuildingRefresh, {core.jmpTo(beginUpdate), 0x90, 0x90})
  core.writeCode(A.BuildingRefreshTile, {core.jmpTo(visitTile), 0x90, 0x90})
  core.writeCode(A.MapReset, {core.jmpTo(resetMap), 0x90})
  core.writeCode(A.TowerFoundationSelect, {core.jmpTo(foundation), 0x90})
end
